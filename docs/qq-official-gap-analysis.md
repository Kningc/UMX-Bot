...
当前实现已经闭环本报告的 P0、P1 和运行治理代码项：统一 OpenAPI Client、
结构化错误、消息回执、完整事件信封、可靠 Gateway 进度、关系事件、显式投递
策略、Outbox、撤回、输入状态、流式回复、完整 Keyboard、分片上传和 TTL 缓存
均已落地。生产凭据验证仍是发布门禁，而不是普通单元测试的一部分。
...
| Gateway 可靠进度 | 已完成 | received/processed sequence 分离；处理成功后提交；SQLite 恢复与 Invalid Session 清理 |
| GROUP_AND_C2C 事件 | 已完成 | 关系、进退群、接收开关、Interaction 映射；未知事件进入 `platform.event` |
| 主动消息治理 | 已完成 | 持久化 Outbox、拒收状态持久化、自适应 429 阻断、非幂等发送不盲重试 |
| 高级消息 | 已完成 | 引用、输入状态、流式 append/replace/complete、Keyboard 三类动作和模板 |
| 富媒体 | 已完成 | 200 MiB 硬限制、自动分片、顺序确认与合并、按场景/会话/摘要隔离的 TTL 缓存 |
| 可观测性 | 已完成 | 脱敏请求日志与 OpenAPI/Gateway 诊断指标通过 Kernel health 暴露 |
...
5. 限频、拒收、权限不足和内容违规不会被错误重试。
6. 进程重启不会无条件丢弃可 Resume 的 Gateway 进度。
7. 自定义按钮、流式消息和富媒体失败均有可诊断错误。
